...
<?php
		...
		создает проблему которую я не решил). После позиционирования алгоритм реализует поиск ближайшей целой записи,
		считывания ее и сравниения с искомым ключом. В соответствии с принципом бинарного поиска переходит на 
		соответствующую половину и все повторяется.

		1. Получает общий размер файла в байтах, вычисляем середину.
		2. Открываем файл переходим в середину файла (функция seek()).
		3. Блочно прочитываем 100 байт с текущей позиции(по условию задачи максимальная запись 4000 байт, 
		можно 100 поменять на 4000 байт).
